...
#   timestamp: 2024-08-09T16:09:26+00:00
...
class Confidence(BaseModel):
    ...
    score: Optional[Annotated[float, Field(strict=True, ge=0.0, le=1.0)]] = Field(
        None,
...
class Occurrence(BaseModel):
    ...
    line: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(
        None,
...
class Cwe(RootModel[conint(ge=1)]):
    root: Annotated[int, Field(strict=True, ge=0)] = Field(
        ...,
...
class Method(BaseModel):
    ...
    confidence: Annotated[float, Field(strict=True, ge=0.0, le=1.0)] = Field(
        ...,
...
class ComponentIdentityEvidence(BaseModel):
    ...
    confidence: Optional[Annotated[float, Field(strict=True, ge=0.0, le=1.0)]] = Field(
        None,
...
class AlgorithmProperties(BaseModel):
    ...
    classicalSecurityLevel: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(
        None,
        description='The classical security level that a cryptographic algorithm provides (in bits).',
        title='classical security level',
    )
    nistQuantumSecurityLevel: Optional[Annotated[int, Field(strict=True, ge=0, le=6)]] = Field(
        None,
...
class License(RootModel[Union[License1, License2]]):
    root: Union[License1, License2] = Field(..., title='License')


class LicenseChoiceItem(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    license: License
...
class LicenseChoice(
    RootModel[Union[List[LicenseChoiceItem], List[LicenseChoiceItem1]]]
):
    root: Union[List[LicenseChoiceItem], List[LicenseChoiceItem1]] = Field(
        ...,
    ...
    def check_root(cls, v):
        if isinstance(v, list):
            if len(v) != 1:
                if all(isinstance(i, LicenseChoiceItem) for i in v):
                    return v
                else:
                    raise ValueError('root must be either a list of SPDX licenses and/or named licenses or a single SPDX License Expression')
            else:
                if isinstance(v[0], LicenseChoiceItem1) or isinstance(v[0], LicenseChoiceItem):
                    return v
                else:
                    raise ValueError('root must be either a list of SPDX licenses and/or named licenses or a single SPDX License Expression')
        else:
...
class DataGovernanceResponsibleParty1(BaseModel):
    ...
    organization: OrganizationalEntity = Field(..., title='Organization')
...
class DataGovernanceResponsibleParty2(BaseModel):
    ...
    contact: OrganizationalContact = Field(..., title='Individual')
...
class Trigger(BaseModel):
    ...
    conditions: Optional[List[Condition]] = Field(None, title='Conditions')
...
class Declarations(BaseModel):
    ...
    affirmation: Optional[Affirmation] = Field(None, title='Affirmation')
    signature: Optional[Signature] = Field(
        None,
        description='Enveloped signature in [JSON Signature Format (JSF)](https://cyberphone.github.io/doc/security/jsf.html).',
        title='Signature',
...
class CyclonedxBillOfMaterialsStandard(BaseModel):
    ...
    version: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(
        1,
